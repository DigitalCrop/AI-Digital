...
const joinCode = () => location.pathname.match(/^\/join\/([A-Za-z2-9]{5})\/?$/)?.[1]?.toUpperCase() || '';
const validRoute = () => location.pathname === '/' || Boolean(joinCode());
...
export function App() {
  ...
  const enter=async(event,payload)=>run(async()=>{const response=await request(event,payload);localStorage.setItem('timepass.session',JSON.stringify({code:response.code,token:response.token}));history.replaceState({},'',`/join/${response.code}`);});
  const leave=async()=>{try{await request(EVENTS.LEAVE_ROOM);}catch{/* already gone */}localStorage.removeItem('timepass.session');setRoom(null);history.replaceState({},'','/');};
  const emit=(event,payload={})=>run(()=>request(event,payload)).catch(()=>{});
  if(!validRoute())return <><Header/><main className="not-found"><span>🪁</span><h1>That page wandered off.</h1><p>Let’s get you back to the games.</p><a className="primary" href="/">Back home</a></main></>;
  if(!room && /room (?:not found|is full)/i.test(error)) return <><Header/><RoomProblem full={/full/i.test(error)} onBack={()=>{setError('');history.replaceState({},'','/');}}/></>;
